...
int main(){
    ...
    if(myfile.is_open()){
        while(getline(myfile, myline)){
            ...
            //up
                ...
                cd_sum += cd.top();
                calculation = REQUIRED_SPACE-(TOTAL_DISK_SPACE-cd_sum);
                if((calculation < total_sum) && (calculation <= -REQUIRED_SPACE)){
                    total_sum = cd_sum;
                }
                cd.pop();
    ...
    cout << "Total size of smallest directory to be cleared: " << total_sum  << endl;
    myfile.close();
    return 0;
}
